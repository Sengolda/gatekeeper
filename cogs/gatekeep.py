    ...
    async def on_member_join(self, member: discord.Member):
        if member.id not in self.verified:
            ...
            rand_word = random.choice(
                ["Tyeoe,sz", "sjindihe", "OWOOWDDN", "6rxdds", "78gxdd"]
            )
            await ch.send(
                f"{member.mention} Yo welcome to bot's gatekeep please type the following\n{str(rand_word)}\n\nAlso this is case-sensitive."
            )
